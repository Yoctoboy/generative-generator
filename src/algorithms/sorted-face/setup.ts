...
const startOrEndConditionColored = (
    ...
    const averageShade =
        (currentPixelShade[0] + currentPixelShade[1] + currentPixelShade[2]) /
        3;
    if (shadesAmount == 0)
        // no current line
...
const drawPure = (p5: P5CanvasInstance, matrixToFollow: PixelMatrix) => {
    for (
        ...
        if (Math.random() < randomAround(linesToSkipRatioAverage, 0.5))
            continue; // skip a line from time to time
